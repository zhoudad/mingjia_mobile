...
class ActionBar extends Component {
  ...
  renderChcek(index, title) {
    ...
    if (this.state.selectIndex[activityIndex] == index) {
      ...
      return (
        <View style={{ flex: 1, justifyContent: "space-between", alignItems: "center", paddingHorizontal: 15, flexDirection: "row", height: 90 * unitWidth, }} >
          <Text style={[styles.item_text_style,
          ...
          <Image
            source={checkImage}
            style={{ width: 24 * unitWidth, height: 16 * unitWidth, tintColor: this.props.activityTintColor ? this.props.activityTintColor : this.defaultConfig.activityTintColor }}
          />
        </View>
      );
    } else {
      return (
        <View
          style={{ flex: 1, justifyContent: "space-between", alignItems: "center", paddingHorizontal: 15, flexDirection: "row" }} >
          <Text
    ...
    if (this.state.activityIndex >= 0) {
      ...
      return (
        <View
          ...
          <ScrollView
            style={[{ position: "absolute", top: 0, left: 0, right: 0, backgroundColor: "white", borderBottomColor: '#ddd', borderBottomWidth: 1 }, heightStyle]}>
            {currentTitles.map((title, index) => (
              ...
              >
                ...
                <View style={{ backgroundColor: "red ", height: 1, marginLeft: 15 }} />
              </Touchable>
            ))}
          </ScrollView>
        </View>
  ...
  render() {
    return (
      <View style={{ flex: 1 }}>
        <View
          style={{ flexDirection: "row",  backgroundColor: this.props.bgColor
              ? this.props.bgColor
        ...
        >
          {this.props.data.map((rows, index) => (
            <Touchable
              ...
              key={index}
              style={{  flex: 1, height: px(90), alignItems: "center", justifyContent: "center", }} >
              <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "center", }} >
                <Text
                  style={[ styles.title_style, this.props.titleStyle,
                    {
